...
6. `ORDER BY` only in the final `SELECT`, never inside a CTE. A top-N inside a
   ...
   it is doing.
7. **Every ordering ends with a tie-breaker** — the tag name, the check name —
   in the final `ORDER BY` and inside every `ROW_NUMBER()`. Ties resolved by
...
9. Every query is layered top to bottom in the same order:
...
10. **Constants live in one place and are derived where they can be.**
    ...
    a constant `CROSS JOIN`s the table; a `params` CTE at the top of a query
    only ever *reads* from those tables. No magic numbers in the middle of a
    query, and nothing to edit by hand if the source is reloaded.
...
18. **Read each source table once, narrow, and materialize.** `posts_questions`
    is read once with `body` (the expensive pass) and once more with four
    narrow columns for asker history; `posts_answers`, `votes`, `post_history`
    and `post_links` are each read once. Everything else in the repository
    reads the materialized tables in `so_analysis`, measured in megabytes.
19. Column pruning is the only real cost lever on this dataset. These public
    ...
    small or a join honest, never to save bytes, and its comment says so.
